...
def test_budget_warning_refresh_is_deferred_after_dialog_exec():
    ...
    assert 'self._schedule_refresh_all_tabs(reason="budget warnings dialog closed")' in src
...
def test_adjustment_dialog_uses_scheduled_parent_refresh():
    ...
    assert 'parent._schedule_refresh_all_tabs(reason="budget adjustment applied")' in src

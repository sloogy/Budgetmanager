# BudgetManager 3.1.1 – Manuel utilisateur

Ce manuel décrit les fonctions réellement disponibles dans la version 3.1.1. BudgetManager conserve les données localement, ne crée aucune écriture sans validation et sépare **budget (prévision)** et **suivi (opérations réelles)**.
...
**Le dossier de données n’a pas sa place dans un dossier synchronisé.** La base, `-wal` et `-shm` forment un tout ; un service de synchronisation pose des verrous dessus, les synchronise indépendamment et peut les décharger. Cela se manifeste par « database is locked », « disk image is malformed » ou, sans message, par des données périmées. L’installateur Windows propose donc `%LOCALAPPDATA%\BudgetManager`, un chemin qu’OneDrive ne couvre jamais. Si votre dossier de données se trouve dans OneDrive — avec la sauvegarde de dossiers activée, cela inclut « Documents » —, BudgetManager le signale une seule fois au démarrage et propose de le déplacer. Le déplacement passe par la même reprise contrôlée que ci-dessus : confirmation, sauvegarde de sécurité, copie et non déplacement. L’ancien dossier reste intact. Un refus est mémorisé.

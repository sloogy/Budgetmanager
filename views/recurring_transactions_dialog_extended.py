...
class RecurringTransactionEditDialog(QDialog):
    ...
    def _setup_ui(self):
        ...
        self.typ_combo.addItem(display_typ(TYP_EXPENSES), TYP_EXPENSES)
        self.typ_combo.addItem(display_typ(TYP_INCOME), TYP_INCOME)
    ...
    def _on_typ_changed(self, _display_text: str):
        """Aktualisiert Kategorien basierend auf Typ"""
        typ = self.typ_combo.currentData() or _display_text
    ...
    def _load_transaction_data(self):
        ...
        # Setze Typ via userData (DB-Key)
        _idx = self.typ_combo.findData(self.transaction.typ)
        if _idx >= 0:
    ...
    def get_data(self) -> dict:
        ...
        data = {
            'typ': self.typ_combo.currentData() or self.typ_combo.currentText(),

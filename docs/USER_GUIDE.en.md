# BudgetManager 3.0.7 – User guide

This guide describes the functions actually available in version 3.0.7. BudgetManager stores data locally, never books without confirmation, and separates **budget (plan)** from **tracking (real transactions)**.
...
**Do not keep the data folder in a cloud-synced folder.** The database, `-wal` and `-shm` belong together; a sync service holds locks on them, syncs them independently and may dehydrate them. You see this as "database is locked", "disk image is malformed" or silently as stale data. The Windows installer therefore proposes `%LOCALAPPDATA%\BudgetManager`, a path OneDrive never covers. If your data folder is inside OneDrive — with folder backup enabled this includes "Documents" — BudgetManager says so once at startup and offers to move it. The move uses the same controlled migration as above: confirmation, safety backup, copy instead of move. The old folder is left untouched. Declining is remembered.

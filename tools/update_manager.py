        ...
        if settings_file.exists():
            try:
                ...
                logging.getLogger(__name__).warning(
                    "Update-Settings defekt (%s) — Defaults werden verwendet: %s",
                    settings_file,
                    e,
                )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--beta", action="store_true", help="Beta-Versionen einschließen"
    )
        ...
        if update_info:
            ...
            if args.download:
                ...
                def progress(downloaded, total):
                    ...
                    print(
                        f"\rFortschritt: {percent:.1f}% ({downloaded}/{total} Bytes)",
                        end="",
                    )

...
class CategoryPropertiesDialog(QDialog):
    ...
    def __init__(self, parent=None, *, conn: sqlite3.Connection, 
        ...
        if not self.category:
            QMessageBox.warning(self, tr("dlg.hinweis"), trf("msg.kategorie_nicht_gefunden_name", category_name=category_name))
            self.reject()
            return
...
class QuickCategoryDialog(QDialog):
    ...
    def __init__(self, parent=None, *, conn: sqlite3.Connection,
                 default_typ: str = tr("kpi.expenses"), default_name: str = ""):
        super().__init__(parent)
        self.conn = conn
        self.cat_model = CategoryModel(conn)
    ...
    def get_created_typ(self) -> str:
        return self.typ_combo.currentData() or db_typ_from_display(self.typ_combo.currentText())

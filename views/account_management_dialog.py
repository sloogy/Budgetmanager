...
class AccountManagementDialog(QDialog):
    ...
    def __init__(
        ...
        *,
        ...
        header = QLabel(
            trf(
                "auto.views_account_management_dialog.58_value_0_ce49c534",
                value_0=(self.user.display_name),
            )
        )
        ...
        status.setStyleSheet(
            f"color: {ui_colors(self).text_dim}; font-size: 11px; margin-bottom: 6px;"
        )
        ...
        tabs.addTab(
            ...
            tr("auto.views_account_management_dialog.78_profil_a112e8f0"),
        )
        tabs.setTabIcon(0, get_icon("✏️"))
        tabs.addTab(
            self._build_secret_tab(),
            tr("auto.views_account_management_dialog.80_passwort_pin_2e7b87ff"),
        )
    ...
    def _build_profile_tab(self) -> QWidget:
        ...
        lbl_username.setStyleSheet(
            f"color: {ui_colors(self).text_dim}; font-size: 11px;"
        )
        ...
        btn_save_name = QPushButton(
            tr("auto.views_account_management_dialog.123_namen_speichern_3c32107f")
        )
        ...
        btn_save_name.setStyleSheet(
            themed(
                ...
                """
            QPushButton {
                padding: 10px 20px; background: %(accent)s; color: %(selection_text)s;
    ...
    def _on_save_display_name(self):
        ...
        if new_name == self.user.display_name:
            show_info(
                self,
                tr("auto.views_account_management_dialog.145_info_80dc0679"),
                tr("account.der_name_ist_unveraendert"),
            )
            return
        ...
        if ok:
            ...
            self._header_label.setText(
                trf(
                    "auto.views_account_management_dialog.151_value_0_3a8da07a",
                    value_0=(new_name),
                )
            )
            ...
            show_info(
                self,
                tr("auto.views_account_management_dialog.153_gespeichert_1672f513"),
                trf("account.anzeigename_geaendert_zu_new_name", new_name=new_name),
            )
        else:
            QMessageBox.critical(
                self, tr("msg.error"), tr("account.name_konnte_nicht_gespeichert")
            )
    ...
    def _build_secret_tab(self) -> QWidget:
        ...
        if self.user.is_quick:
            info = QLabel(
                tr(
                    "auto.views_account_management_dialog.172_dein_konto_hat_aktuell_b_keinen_pas_1282d8ba"
                )
            )
            info.setWordWrap(True)
            info.setStyleSheet(
                f"padding: 15px; background: {ui_colors(self).warning_bg}; border-radius: 5px;"
            )
            ...
            lbl = QLabel(
                tr(
                    "auto.views_account_management_dialog.186_b_nur_an_einem_sicheren_ort_speiche_b5321905"
                )
            )
            ...
            if self.db_key:
                ...
                try:
                    ...
                    self.txt_restore_key.setPlainText(
                        db_key_to_restore_key(self.db_key)
                    )
                except Exception:
        ...
        layout.addWidget(
            QLabel(
                trf("account.aendere_dein_aktuelles_mode_label", mode_label=mode_label)
            )
        )
        layout.addSpacing(5)
        ...
        self.edt_old_secret.setPlaceholderText(
            trf(
                "auto.views_account_management_dialog.237_aktuelles_value_0_eingeben_53722645",
                value_0=(mode_label),
            )
        )
        if is_pin:
        ...
        form.addRow(
            trf("account.current_secret_label", mode_label=mode_label),
            self.edt_old_secret,
        )
        ...
        if is_pin:
            ...
            self.edt_new_secret.setPlaceholderText(
                tr("create_user.password_placeholder")
            )
        form.addRow(
            trf("account.new_secret_label", mode_label=mode_label), self.edt_new_secret
        )
        ...
        self.edt_new_secret2.setPlaceholderText(
            trf(
                "auto.views_account_management_dialog.257_value_0_wiederholen_679c9574",
                value_0=(mode_label),
            )
        )
        if is_pin:
        ...
        btn_change = QPushButton(
            trf("account.mode_label_aendern", mode_label=mode_label)
        )
        btn_change.setStyleSheet(
            themed(
                ...
                """
            QPushButton {
                padding: 10px 20px; background: %(warning)s; color: %(selection_text)s;
                border: none; border-radius: 5px; font-weight: bold; font-size: 13px;
            }
            QPushButton:hover { background: %(warning_hover)s; }
        """,
            )
        )
    ...
    def _copy_restore_key(self) -> None:
        ...
        try:
            ...
            if not key or key.startswith("("):
                show_warning(
                    self,
                    tr("backup.restore_key_dialog_title"),
                    tr("account.kein_restorekey_verfuegbar"),
                )
                return
            ...
            show_info(
                self,
                tr("backup.restore_key_dialog_title"),
                tr("account.restorekey_wurde_kopiert"),
            )
        except Exception:
            show_warning(
                self,
                tr("backup.restore_key_dialog_title"),
                tr("account.kopieren_nicht_moeglich"),
            )
    ...
    def _on_change_secret(self):
        ...
        if new_secret != new_secret2:
            show_warning(
                ...
                tr("dlg.hinweis"),
                trf(
                    "account.die_mode_labeleingaben_stimmen_nicht",
        ...
        if is_pin:
            if not new_secret.isdigit() or not (
                PIN_MIN_LENGTH <= len(new_secret) <= PIN_MAX_LENGTH
            ):
                show_warning(self, tr("dlg.hinweis"), tr("account.pin_length"))
                return
        else:
            if len(new_secret) < PASSWORD_MIN_LENGTH:
                show_warning(self, tr("dlg.hinweis"), tr("account.password_min_length"))
                return
        ...
        if success:
            ...
            if restore_key:
                ...
                show_info(
                    ...
                    tr("msg.success"),
                    trf(
                        "account.mode_label_wurde_erfolgreich_geaendert",
                        mode_label=mode_label,
                    ),
                )
        else:
    ...
    def _build_security_tab(self) -> QWidget:
        ...
        layout.addWidget(
            QLabel(
                tr(
                    "auto.views_account_management_dialog.360_wechsle_die_sicherheitsstufe_deines_f6de7988"
                )
            )
        )
        layout.addSpacing(5)
        ...
        new_box = QGroupBox(
            tr(
                "auto.views_account_management_dialog.378_neue_sicherheitsstufe_3f613920"
            )
        )
        ...
        self.rb_sec_quick.setToolTip(
            tr(
                "auto.views_account_management_dialog.385_schneller_zugriff_ohne_eingabe_schu_5c56399c"
            )
        )
        ...
        self._lbl_sec_old = QLabel(
            tr("auto.views_account_management_dialog.415_aktuelles_passwort_38b1f100")
        )
        ...
        self._lbl_sec_new = QLabel(
            tr("auto.views_account_management_dialog.427_neues_passwort_d5fba2a0")
        )
    ...
    def _on_security_selection_changed(self):
        ...
        if needs_old:
            current_kind = display_secret_kind(
                "pin" if self.user.is_pin else "password"
            )
        ...
        if needs_new:
            ...
            ph = (
                tr("create_user.pin_placeholder")
                if is_pin_target
                else tr("create_user.password_placeholder")
            )
        ...
        if target == SECURITY_QUICK:
            if current != SECURITY_QUICK:
                self.lbl_sec_warn.setText(
                    tr(
                        "auto.views_account_management_dialog.513_quick_modus_entfernt_den_passwortsc_84601067"
                    )
                )
                self.lbl_sec_warn.setStyleSheet(
                    f"color: {ui_colors(self).negative}; font-size: 11px; padding: 5px;"
            ...
            self.lbl_sec_warn.setText(
                trf(
                    "auto.views_account_management_dialog.525_value_0_oder_restore_key_verlieren__81330d27",
                    value_0=(kind),
                )
            )
            self.lbl_sec_warn.setStyleSheet(
        ...
        if same_level:
            self.btn_sec_apply.setText(
                tr("auto.views_account_management_dialog.538_aktuelle_stufe_5d6746d9")
            )
            ...
            self.btn_sec_apply.setText(
                trf(
                    "auto.views_account_management_dialog.542_wechseln_zu_value_0_c4af1f8b",
                    value_0=(target_label),
                )
            )
    ...
    def _on_change_security(self):
        ...
        if target == current:
            show_info(
                self,
                tr("auto.views_account_management_dialog.558_info_c90436c3"),
        ...
        if target != SECURITY_QUICK:
            ...
            if not new_secret:
                kind = display_secret_kind(
                    "pin" if target == SECURITY_PIN else "password"
                )
                show_warning(
                    self,
                    tr("dlg.hinweis"),
            ...
            if target == SECURITY_PIN:
                ...
                if len(new_secret) < PASSWORD_MIN_LENGTH:
                    show_warning(
                        self, tr("dlg.hinweis"), tr("account.password_min_length")
                    )
                    return
        ...
        if target == SECURITY_QUICK:
            reply = QMessageBox.warning(
                self,
                tr(
                    "auto.views_account_management_dialog.606_schutz_entfernen_0233ca15"
                ),
        ...
        if success:
            ...
            if restore_key:
                ...
                show_info(
                    ...
                    tr("msg.success"),
                    trf(
                        "auto.views_account_management_dialog.653_sicherheitsstufe_geaendert_zu_value_de978174",
                        value_0=(target_label),
                    ),
                )
        else:
    ...
    def _show_restore_key(self, restore_key: str, context: str = ""):
        ...
        header = QLabel(
            trf(
                "account.context_erfolgreich_geaendert",
                context=(context or tr("dlg.restore_key_new")),
            )
        )
        hf = QFont()
        hf.setPointSize(13)
        ...
        warn = QLabel(
            tr(
                "auto.views_account_management_dialog.692_b_wichtig_b_dein_restore_key_hat_si_aa706b0f"
            )
        )
        ...
        warn.setStyleSheet(
            themed(
                self,
                "background: %(warning_bg)s; padding: 10px; border-radius: 5px; "
        ...
        layout.addWidget(
            QLabel(
                tr(
                    "auto.views_account_management_dialog.705_dein_neuer_restore_key_0417e1e7"
                )
            )
        )
        ...
        key_edit.setStyleSheet(
            themed(
                ...
                "font-size: 13px; padding: 8px; background: %(bg_panel)s; "
                "border: 2px solid %(border)s; border-radius: 4px;",
            )
        )
        ...
        btn_copy = QPushButton(
            tr(
                "auto.views_account_management_dialog.720_in_zwischenablage_kopieren_a988f8aa"
            )
        )
        btn_copy.setIcon(get_icon("📋"))
        btn_copy.setStyleSheet(
            themed(
                ...
                """
            QPushButton {
                padding: 8px 16px; background: %(accent)s; color: %(selection_text)s;
        ...
        btn_ok = QPushButton(
            tr(
                "auto.views_account_management_dialog.736_ich_habe_den_key_notiert_9961ef11"
            )
        )
        ...
        btn_ok.setStyleSheet(
            themed(
                ...
                """
            QPushButton {
                padding: 10px 24px; background: %(positive)s; color: %(selection_text)s;
                border: none; border-radius: 5px; font-weight: bold; font-size: 13px;
            }
            QPushButton:hover { background: %(positive_hover)s; }
        """,
            )
        )
    ...
    def _copy_to_clipboard(self, text: str, btn: QPushButton):
        ...
        QTimer.singleShot(
            ...
            lambda: (
                btn.setText(
                    tr(
                        "auto.views_account_management_dialog.757_in_zwischenablage_kopieren_a4a61e69"
                    )
                ),
                btn.setIcon(get_icon("📋")),
            ),

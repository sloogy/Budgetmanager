...
### Bankimport UX-V4 – 24. August 2026

- Der aktive Bankimport wurde als eigenständige V4-Oberfläche neu aufgebaut. V2/V3 bleiben nur noch als Legacy-Referenz; die Runtime startet V4.
- Die Haupttabelle hat nur noch sieben Spalten: Auswahl, Datum, Buchung, Betrag, Kategorie, Quelle und Status. Typ, KI-Technik und Tags sind aus der Hauptansicht entfernt.
- Es gibt nur noch eine Auswahl: das Import-Häkchen. Dasselbe Häkchen steuert Massenaktionen; eine getrennte blaue Tabellen-Selektion existiert nicht mehr.
- Statusfilter `Zu prüfen`, `Bereit`, `Bereits erledigt` und `TWINT` machen offene Fälle zum primären Arbeitsfluss. Suche und Sortierung bleiben erhalten.
- Der Buchungstyp folgt automatisch der gewählten Kategorie. Unsichere KI-Vorschläge können durch erneutes Auswählen derselben Kategorie bestätigt werden.
- Kategorie-Tags werden automatisch übernommen. Optionale Tags liegen hinter einem einzigen `Tags…`-Dialog; bei Mehrfachauswahl bedeutet ein halb gesetztes Häkchen `gemischt / unverändert`.
- Mehrere Dateien können gemeinsam geladen und versehentlich geladene Dateien wieder entfernt werden. Identische Dateien werden innerhalb desselben Reviews nicht doppelt zugelassen.
- TWINT-Eingänge bleiben reine Lern-/Erstattungssignale mit 0.00 Budgetwirkung. Verrechnung bleibt Opt-in; sichere Treffer übernehmen die Kategorie der zugehörigen Ausgabe automatisch.

### Undo/Redo-Hotfix – 24. August 2026

- Rückgängig/Wiederholen bleibt global auslösbar und ist zusätzlich direkt in der Aktionsleiste verfügbar.
- Budget-Saves erzeugen keine Undo-Einträge mehr für unveränderte Werte; dadurch verschwinden scheinbar wirkungslose Rückgängig-Schritte.
- Mehrmonatige Budgetänderungen werden als eine Undo-Gruppe behandelt, sodass ein Benutzer-Schritt auch mit einem Ctrl+Z zurückgenommen wird.
- Eine bearbeitete Jahressumme wird bei Auto-Save sofort als zusammenhängende Undo-Aktion gespeichert.
- Vor Undo/Redo wird ein noch aktiver Budget-Zelleditor sauber abgeschlossen, damit unter Windows nicht ein noch ungecommiteter UI-Wert an der History vorbeiläuft.
...
- Der ursprüngliche 3.0.5-Stand entsprach zunächst 3.0.4; der Hotfix **Bankimport UX-V4 – 24. August 2026** ersetzt diese Oberfläche vollständig.

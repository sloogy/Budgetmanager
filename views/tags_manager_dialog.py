...
class TagsManagerDialog(QDialog):
    ...
    def _setup_ui(self):
        ...
        info = QLabel(
            tr(
                "auto.views_tags_manager_dialog.45_tags_ermoeglichen_eine_zusaetzliche_115e8eae"
            )
        )
        ...
        self.table.setHorizontalHeaderLabels(
            [
                tr("lbl.day"),
                tr("lbl.color"),
                tr("tags.action_text_header"),
                tr("auto.views_tags_manager_dialog.54_anzahl_verwendungen_838041ae"),
                tr("auto.views_tags_manager_dialog.54_aktionen_3bb76da2"),
            ]
        )
        ...
        self.btn_add.setToolTip(
            tr("auto.views_tags_manager_dialog.75_neues_tag_erstellen_08b57a81")
        )
        self.btn_add.clicked.connect(self._add_tag)
        btn_layout.addWidget(self.btn_add)

        self.btn_edit = QPushButton(
            tr("auto.views_tags_manager_dialog.79_bearbeiten_86a45c89")
        )
        ...
        self.btn_stats = QPushButton(
            tr("auto.views_tags_manager_dialog.99_statistiken_6940bacd")
        )
        self.btn_stats.setIcon(get_icon("📊"))
        self.btn_stats.setToolTip(
            tr("auto.views_tags_manager_dialog.101_tag_statistiken_anzeigen_48ad4b4e")
        )
    ...
    def _load_tags(self):
        ...
        for tag in tags:
            ...
            tag_name = tag["name"]
            color = tag.get("color", "")
            action_text = str(tag.get("action_text", "") or "")
            ...
            if color:
                ...
                color_item.setText(
                    tr("auto.views_tags_manager_dialog.168_keine_8f599208")
                )
            self.table.setItem(row, 1, color_item)

            # Aktionstext / Details-Vorlage
            action_item = QTableWidgetItem(action_text)
            action_item.setToolTip(tr("tags.action_text_tip"))
            ...
            btn_color.setFixedSize(36, 32)
            btn_color.setToolTip(tr("dlg.farbe_aendern"))
            btn_color.clicked.connect(
                lambda checked, tid=tag_id: self._change_color(tid)
            )
    ...
    def _add_tag(self):
        ...
        if reply == QMessageBox.Yes:
            qcolor = QColorDialog.getColor(
                QColor(ui_colors(self).accent), self, tr("msg.tag_farbe_waehlen")
            )
            if qcolor.isValid():
                color = qcolor.name()

        # Tag erstellen
        tag_id = self.tags_model.create_tag(name, color, action_text.strip())
    ...
    def _edit_tag(self):
        ...
        if new_name != old_name and self._tag_name_exists(new_name):
            show_warning(
                ...
                trf(
                    "auto.views_tags_manager_dialog.284_ein_tag_mit_dem_namen_value_0_exist_126d57ed",
                    value_0=(new_name),
                ),
            )
            return
        ...
        # Tag umbenennen
        success = self.tags_model.update_tag(tag_id, new_name, action_text.strip())
        ...
        if success:
            ...
            show_info(
                ...
                tr("msg.success"),
                trf(
                    "auto.views_tags_manager_dialog.296_tag_wurde_umbenannt_in_value_0_a29907d3",
                    value_0=(new_name),
                ),
            )
        else:
            show_warning(
                ...
                tr("msg.error"),
                tr(
                    "auto.views_tags_manager_dialog.302_tag_konnte_nicht_umbenannt_werden_a89177d2"
                ),
            )
    ...
    def _delete_tag(self):
        ...
        usage_count = int(self.table.item(current_row, 3).text())
        ...
        if success:
            ...
            show_info(
                ...
                tr("msg.success"),
                trf(
                    "auto.views_tags_manager_dialog.349_tag_value_0_wurde_geloescht_b148924f",
                    value_0=(tag_name),
                ),
            )
        else:
            show_warning(
                ...
                tr("msg.error"),
                tr(
                    "auto.views_tags_manager_dialog.355_tag_konnte_nicht_geloescht_werden_8f83bd72"
                ),
            )
    ...
    def _change_color(self, tag_id: int):
        ...
        # Farbdialog
        initial_color = (
            current_color if current_color else QColor(ui_colors(self).accent)
        )
        qcolor = QColorDialog.getColor(
            initial_color, self, trf("tags.dialog.color_title", tag=tag_name)
        )
        ...
        if success:
            ...
            show_warning(
                ...
                tr("msg.error"),
                tr(
                    "auto.views_tags_manager_dialog.391_farbe_konnte_nicht_gespeichert_werd_124397e4"
                ),
            )
    ...
    def _merge_tags(self):
        ...
        if len(tags) < 2:
            show_info(
                ...
                tr(
                    "auto.views_tags_manager_dialog.403_es_muessen_mindestens_2_tags_vorhan_656a7014"
                ),
            )
            return
        ...
        info = QLabel(
            tr(
                "auto.views_tags_manager_dialog.417_waehlen_sie_die_tags_aus_die_zusamm_8f9053ec"
            )
        )
        info.setWordWrap(True)
        layout.addWidget(info)
        ...
        if target_id == source_id:
            show_warning(
                ...
                tr(
                    "auto.views_tags_manager_dialog.459_ziel_und_quelle_duerfen_nicht_ident_8347b5bc"
                ),
            )
            return
        ...
        if success:
            ...
            show_info(
                ...
                tr("msg.success"),
                tr(
                    "auto.views_tags_manager_dialog.471_tags_wurden_erfolgreich_zusammengef_c25182ec"
                ),
            )
        else:
            show_warning(
                ...
                tr("msg.error"),
                tr(
                    "auto.views_tags_manager_dialog.477_tags_konnten_nicht_zusammengefuehrt_5b23f4a0"
                ),
            )
    ...
    def _show_stats(self):
        ...
        if not stats:
            show_info(
                ...
                tr(
                    "auto.views_tags_manager_dialog.488_es_sind_noch_keine_tags_mit_buchung_502ddf34"
                ),
            )
            return
        ...
        table.setHorizontalHeaderLabels(
            [
                tr("lbl.day"),
                tr("auto.views_tags_manager_dialog.501_anzahl_buchungen_d50207ae"),
                tr("auto.views_tags_manager_dialog.501_gesamtbetrag_034bb324"),
            ]
        )

...
i18n.set_language("de"); money.set_currency("CHF"); money.set_number_format("ch")
check(money.format_money(1234.5) == "1'234.50 CHF", f"Format CHF/ch: {money.format_money(1234.5)!r}")
# DAU mit deutschem Euro-Format
money.set_currency("EUR"); money.set_number_format("eu")
check(money.format_money(1234.5) == "1.234,50 €", f"Format EUR/eu: {money.format_money(1234.5)!r}")
money.set_currency("CHF"); money.set_number_format("ch")
...
try:
    ...
    check(False, f"User-Anlage Exception: {e}"); traceback.print_exc()
...
check(isinstance(tree, list) and len(tree) > 0, f"Kategorien-Baum baubar ({len(tree)} Wurzeln)")
...
try:
    ...
    val = conn.execute("SELECT amount FROM budget WHERE year=2026 AND month=1 AND typ=? AND category=?",
                       (sample.typ, sample.name)).fetchone()[0]
...
try:
    ...
    cnt = conn.execute("SELECT COUNT(*) FROM tracking WHERE category=?", (sample.name,)).fetchone()[0]
...
try:
    cats.rename_and_cascade(sample.id, typ=sample.typ, old_name=sample.name, new_name="DAU-Umbenannt")
    b = conn.execute("SELECT COUNT(*) FROM budget WHERE category='DAU-Umbenannt'").fetchone()[0]
    t = conn.execute("SELECT COUNT(*) FROM tracking WHERE category='DAU-Umbenannt'").fetchone()[0]
    rest = conn.execute("SELECT COUNT(*) FROM budget WHERE category=?", (sample.name,)).fetchone()[0]
    check(b==1 and t==1 and rest==0, f"Rename-Cascade ok (budget={b}, tracking={t}, Reste={rest})")
except Exception as e:
...
try:
    parent = next((c for c in cats.list() if any(ch.parent_id==c.id for ch in cats.list())), None)
    if parent:
        child_ids_before = [c.id for c in cats.list() if c.parent_id==parent.id]
        cats.delete_category_safely(parent.id, data_action="delete_until_last_booking", promote_children=True)
...
try:
    ...
    orphans = 0
    for table in ("budget","tracking","budget_warnings","favorites","recurring_transactions","suggestion_accepted"):
        for r in conn.execute(f"SELECT DISTINCT typ, category FROM {table}").fetchall():
            if (r[0], r[1]) not in cat_names:
                orphans += 1
    # verwaiste parent_id?
    valid_ids = {c.id for c in cats.list()}
    dangling = conn.execute("SELECT COUNT(*) FROM categories WHERE parent_id IS NOT NULL AND parent_id NOT IN (%s)" %
                            ",".join(str(i) for i in valid_ids or [0])).fetchone()[0]
...
conn.close()
print("\n" + "="*64)
print(f"ERGEBNIS: {'ALLE CHECKS BESTANDEN ✅' if not FAIL else f'{len(FAIL)} FEHLER ❌'}")

...
class FavoritesDashboardDialog(QDialog):
    ...
    def _load_favorites(self):
        ...
        if not favorites:
            ...
            item = QTableWidgetItem(
                tr("favorites.empty") + " " + tr("dlg.fuegen_sie_favoriten_im")
            )
        ...
        for typ, category in favorites:
            ...
            rest = (
                (booked_amount - budget_amount)
                if typ_key == TYP_INCOME
                else (budget_amount - booked_amount)
            )
        ...
        summary_text = trf(
            "lbl.summary_total_budget",
            budget=format_money(total_budget),
            booked=format_money(total_booked),
        )
        if total_rest >= 0:
            summary_text += trf(
                "lbl.summary_remaining", color=c.ok, amount=format_money(total_rest)
            )
        else:
            summary_text += trf(
                "lbl.summary_overrun",
                color=c.negative,
                amount=format_money(abs(total_rest)),
            )
    ...
    def _manage_favorites(self):
        ...
        show_info(self, tr("favorites.manage.title"), tr("favorites.manage.body"))

...
- [ ] **🐛 BUG: Backup sichert falsche Datei (`data/c.enc`, 136 KB statt echte DB)**
  - Neue Backups sind nur ~103 KB, alte korrekte Backups waren 4.9–7.9 MB
  - Ursache: `encrypted_session.enc_path` zeigt auf `data/c.enc` (leer/falsch)
    statt auf die eigentliche Produktionsdatenbank
  - Zu prüfen: wo `_encrypted_session` initialisiert wird + Login-Pfad in `main_window.py`
  - Letzte korrekte Backups: `budgetmanager_backup_20260303_102919.bmr` (7.9 MB) → Daten sicher
  - Diagnose-Datum: 03.03.2026

...
class MainWindow(QMainWindow):
    ...
    def _create_view_menu(self, menubar: QMenuBar) -> None:
        ...
        for pos_key, pos_label in _pos_defs:
            ...
            act.triggered.connect(
                lambda checked, k=pos_key: self._on_tab_position_changed(k)
            )
    ...
    def _set_tab_visible(self, tab_id: int, visible: bool) -> None:
        ...
        if not visible and idx >= 0 and self.tabs.count() <= 1:
            QMessageBox.information(
                self, tr("cockpit.hide_tab_title"), tr("cockpit.keep_one_tab")
            )
    ...
    def _show_settings(self):
        ...
        if dialog.exec() == QDialog.Accepted:
            ...
            self.settings.set(
                "show_onboarding", new_settings.get("show_onboarding", True)
            )
            self.settings.set(
            ...
            self.settings.set(
                "number_format", new_settings.get("number_format", "swiss")
            )
            ...
            try:
                ...
                try:
                    ...
                    from utils.qt_translator import (
                    ...
                    if _app is not None:
                        install_qt_translations(
                            _app, new_settings.get("language", "de")
                        )
            ...
            self.settings.set(
                "table_density", new_settings.get("table_density", "Normal")
            )
            self.settings.set(
            ...
            self.settings.set(
                "backup_days", int(new_settings.get("backup_days", 30) or 30)
            )
            self.settings.set(
        ...
        for path in self._help_file_candidates(rel_path):
            try:
                if path.exists():
                    ...
                    if self.statusBar():
                        self.statusBar().showMessage(
                            trf("msg.help_opened", path=str(path)), 3000
                        )
                    return True
            except Exception as e:
        ...
        try:
            ...
            def _setup_finished(*_args) -> None:
                ...
                if getattr(self, "_defer_startup_auto_backup_until_setup", False):
                    ...
                    logger.info(
                        "Setup-Assistent beendet; verschobenes Auto-Backup wird geprüft."
                    )

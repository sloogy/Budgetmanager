...
class FixcostCheckModel:
    ...
    def _ensure_tables(self):
        ...
        if "is_fixcost" not in columns:
            cursor.execute(
                "ALTER TABLE categories ADD COLUMN is_fixcost INTEGER DEFAULT 0"
            )

        if "expected_monthly_bookings" not in columns:
            cursor.execute(
                "ALTER TABLE categories ADD COLUMN expected_monthly_bookings INTEGER DEFAULT 1"
            )
    ...
    def set_category_as_fixcost(
        self, category_id: int, is_fixcost: bool = True, expected_bookings: int = 1
    ):
        """
    ...
    def get_fixcost_status_for_month(self, year: int, month: int) -> Dict:
        ...
        return {
            ...
            "completion_percentage": (
                (booked_count / total_fixcosts * 100) if total_fixcosts > 0 else 100
            ),

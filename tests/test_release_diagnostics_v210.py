...
def test_diagnostic_report_zip_contains_only_diagnostics_not_user_database(tmp_path, monkeypatch) -> None:
    monkeypatch.setenv("BUDGETMANAGER_APP_DIR", str(tmp_path))
    data = data_dir()
    ...
    diagnostics.runtime_state_path().write_text('{"last_exit_clean": false}', encoding="utf-8")
    settings_path().write_text(
        json.dumps({"theme": "dark", "password": "secret", "nested": {"api_token": "secret-token"}}),
        encoding="utf-8",
    )
    ...
    (data / "backups" / "backup.db").write_text("must not be included", encoding="utf-8")
    ...
    with zipfile.ZipFile(report) as zf:
        ...
        sanitized = json.loads(zf.read("budgetmanager_settings.sanitized.json").decode("utf-8"))
...
def test_help_menu_contains_diagnostic_actions_and_i18n_keys() -> None:
    ...
    for lang in ["de", "en", "fr"]:
        ...
        for key in ["show_log", "show_crash_log", "open_diagnostics_folder", "create_diagnostic_report"]:
            assert key in menu
        for key in ["unclean_title", "show_log", "create_report", "report_created_text"]:
            assert key in diag


def test_unclean_previous_run_does_not_use_pid_liveness(monkeypatch) -> None:
    def _boom(_pid):
        raise AssertionError("PID liveness must not be consulted for crash prompt state")

    monkeypatch.setattr(diagnostics, "_pid_alive", _boom)
    assert diagnostics.previous_state_was_unclean({"app_running": True, "pid": 1}) is True
...
def test_diagnostic_report_zip_writes_manifest_and_read_errors_for_missing_main_log(tmp_path, monkeypatch) -> None:
    monkeypatch.setenv("BUDGETMANAGER_APP_DIR", str(tmp_path))
    report = diagnostics.create_diagnostic_report_zip()
    with zipfile.ZipFile(report) as zf:
        names = set(zf.namelist())
...
def test_diagnostic_report_masks_home_paths_in_runtime_and_system_info(tmp_path, monkeypatch) -> None:
    monkeypatch.setenv("BUDGETMANAGER_APP_DIR", str(tmp_path))
    ...
    diagnostics.runtime_state_path().write_text(
        json.dumps({"argv": [str(tmp_path / "BudgetManager.exe")], "app_running": True}),
        encoding="utf-8",
    )
    report = diagnostics.create_diagnostic_report_zip()
    with zipfile.ZipFile(report) as zf:

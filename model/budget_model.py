...
class BudgetModel:
    ...
    def get_amount(
        ...
        """Liest einen einzelnen Budget-Betrag.

        Wird u.a. im Tracking (Fixkosten/Monatsanfang) benötigt.
        Gibt `default` zurück, wenn kein Eintrag existiert oder die Kategorie reserviert ist.
        """
        # SCHUTZ: Reservierte Kategorien immer als 0 behandeln
        if self._is_reserved_category(category):
    ...
    def sum_by_typ_range(self, year_months: list[tuple[int, int]]) -> dict[str, float]:
        """Budget-Summen nach Typ für eine beliebige Menge von (year, month)-Paaren.

        Args:
            year_months: Liste von (year, month)-Tupeln.

        Returns:
            Dict {typ: summe}, z.B. {TYP_INCOME: 5000.0, TYP_EXPENSES: 3000.0}
        """
        if not year_months:
            return {}
        ...
        for y, mlist in by_year.items():
            ...
            for r in cur.fetchall():
                t = str(r["typ"])
                result[t] = result.get(t, 0.0) + float(r["s"] or 0.0)
        return result

    def sum_by_typ_category_range(
        self, year_months: list[tuple[int, int]]
    ) -> dict[tuple[str, str], float]:
        """Budget-Summen nach (typ, category) für eine beliebige Menge von (year, month)-Paaren.

        Returns:
            Dict {(typ, category): summe}
        """
        if not year_months:
            return {}
        ...
        for y, mlist in by_year.items():
            ...
            for r in cur.fetchall():
                key = (str(r["typ"]), str(r["category"]))
                if not self._is_reserved_category(key[1]):
                    result[key] = result.get(key, 0.0) + float(r["s"] or 0.0)
        return result

    def count(self) -> int:
        """Anzahl der Budget-Einträge."""
        row = self.conn.execute("SELECT COUNT(*) FROM budget").fetchone()

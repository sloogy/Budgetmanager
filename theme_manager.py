...
try:
    from utils.table_autosize import autosize_all_tables
except Exception:  # pragma: no cover
    autosize_all_tables = None
...
class ThemeManager:
    ...
    def get_negative_color(self) -> str:
        """Gibt die Farbe für negative Beträge zurück."""
        p = self.get_current_profile()
        return p.get("negativ_text", "#e74c3c") if p else "#e74c3c"

    def get_severity_color(self, level: str) -> str:
        ...
        return {
            ...
            "warning": "#f39c12",
            "danger": p.get("negativ_text", "#e74c3c"),
        }.get(level, "#333")

    def _fallback_hover(self, sel_bg: str, table_bg: str) -> str:
        """Berechnet eine sinnvolle Hover-Farbe als Mischung aus Auswahl und Tabellenhintergrund."""
        try:
            sb = QColor(sel_bg)
            tb = QColor(table_bg)
            # 25% Selection-Farbe über den Tabellenhintergrund
            r = int(tb.red() * 0.75 + sb.red() * 0.25)
            g = int(tb.green() * 0.75 + sb.green() * 0.25)
            b = int(tb.blue() * 0.75 + sb.blue() * 0.25)
            return QColor(min(r, 255), min(g, 255), min(b, 255)).name()
        except Exception:
            return "#e0e0e0"
    ...
    def build_stylesheet(self, profile: ThemeProfile) -> str:
        ...
        hover_text = p.get("hover_text") or text
...
QTableWidget {{ background-color: {table_bg}; alternate-background-color: {table_alt}; gridline-color: {table_grid}; selection-background-color: {sel_bg}; selection-color: {sel_text}; }}
	...
	QTreeWidget::item:selected:hover, QTreeView::item:selected:hover {{ background-color: {sel_bg}; color: {sel_text}; }}
...
"""
    ...
    def apply_theme(self, app=None, profile_name=None):
        ...
        if profile and app:
            ...
            # Globaler App-Font (wichtig für Tabellen-Metriken)
            try:
                fs = int(profile.get("schriftgroesse", 10) or 10)
                f = app.font()
                f.setPointSize(fs)
                app.setFont(f)
            except Exception as e:
                logger.debug("fs = int(profile.get('schriftgroesse', 10) or 10): %s", e)

            # Tabellen/Views anpassen (Row-Height/Header-Height)
            if autosize_all_tables:
                try:
                    autosize_all_tables(app)
                except Exception as e:
